...
function isProcessFile(path) {
    const ext = extname(path);
    return statSync(path).isFile() && ['.vue', '.jsx', '.tsx'].includes(ext);
}

function isProcessDirectory(path, item) {
...
function getRouteName(parentRoutePath, fileName) {
    ...
    return routeName
        ...
        .replace(/\[([a-z]+)\]/i, '_$1')
        .replace(/\[...([a-z]*)\]/i, 'FUZZYMATCH-$1');
}

function getRoutePath(parentRoutePath, fileName, isFile = true) {
    // /index.vue -> /
    ...
    if (/\[[a-z]+\]/i.test(fileName)) {
        fileName = fileName.replace(/\[([a-z]+)\]/gi, ':$1');
    }
    // /[...slug].vue -> /:slug(.*)
    if (/\[...[a-z]*\]/i.test(fileName)) {
        fileName = fileName.replace(/\[...([a-z]*)\]/i, ':$1(.*)').replace(':(.*)', ':pathMatch(.*)');
    }
...
function genRoutes(parentRoutes, path, parentRoutePath) {
    const dirList = readdirSync(path);
...
function rank(routes) {
    routes.forEach((item) => {
        ...
        arr.forEach((sonPath) => {
            count += 4;
            if (sonPath.includes(':') && !sonPath.includes('(.*)')) {
                count += 2;
            }
            else if (sonPath.includes('(.*)')) {
                count -= 1;
            }
            else if (sonPath === '') {
                count += 1;
...
function getRoutes({ config, absPagesPath }) {
    // 用户配置了routes则使用用户配置的
