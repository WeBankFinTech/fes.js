...
export function getName(absPath, absSrcPath) {
...
export function getPath(absPath) {
...
export function genExtraModels(models = [], absSrcPath) {
...
export function sort(ns) {
    ...
    ns.forEach((item, index) => {
        if (item.use && item.use.length) {
            ...
            if (errorList.length) {
                throw new Error(`Circular dependencies: ${item.namespace} can't use ${errorList.join(', ')}`);
            }
...
export function genModels(imports, absSrcPath) {
    ...
    if (checkDuplicates(contents.map(ele => ele.namespace))) {
        throw new Error('plugin-model: models 中包含重复的 namespace！');
    }
    return raw.sort((a, b) => models.indexOf(a.namespace) - models.indexOf(b.namespace));
}

export function isValidHook(filePath) {
...
export function getValidFiles(files, modelsDir) {
    return files
        .map((file) => {
