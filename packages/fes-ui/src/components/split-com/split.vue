<template>
    <div :class="getClass" class="ui-split">
        <slot />
        <div v-for="i in itemLength" v-if="i > 1" :key="i" :style="getDashStyle(i)" @mousedown.stop="handleMouseDown(i, $event)" class="ui-split-dash" />
    </div>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        fristItem() {
            if (this.moveIndex) {
                const fristItem = this.items.filter(item => item.index == this.moveIndex - 1)[0];
                return fristItem;
            }
        },
        secondItem() {
            if (this.moveIndex) {
                const secondItem = this.items.filter(item => item.index == this.moveIndex)[0];
                return secondItem;
            }
        }
    },
    ...
    methods: {
        ...
        handleMouseMove(event) {
            if (this.moveIndex) {
                ...
                const fristSize = this.fristItem[variable] + this.currentChange;
                const secondSize = this.secondItem[variable] - this.currentChange;
